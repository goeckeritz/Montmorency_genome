...
#mkdir topology_7_bootstrap_nexus_files
cd /mnt/gs21/scratch/goeckeri/orthofinder_montAv5_scaffolded/Genomes/RAxML/polyploidy_event_est/topology_7_bootstrap_nexus_files
...
echo "$(echo "tree $i = $(sed -n $(echo $i)p /mnt/gs21/scratch/goeckeri/orthofinder_montAv5_scaffolded/Genomes/RAxML/polyploidy_event_est/combined_topology7_species_MSAs.fa.raxml.bootstraps)")" >> bs_tree$i.txt
...
echo "
begin r8s;
blformat lengths=persite nsites=78183 ultrametric=no; 
...
cd /mnt/gs21/scratch/goeckeri/orthofinder_montAv5_scaffolded/Genomes/RAxML/polyploidy_event_est/topology_7_bootstrap_nexus_files/r8s_output/
...
sed -n 137,142p $i | awk '{ print $2,$3 }' > ./node_ages/node_ages_$i ################find out what lines these are!
done
...
grep 'subA_fru' 500_bs_node_estimates.txt | awk '{print $2}' > subA_fruticosa_node
...
paste subA_fruticosa_node subB_avium_node sweet_ground_node peach_cherry_node peach_apple_node subA___fruticosa_node > topology7_500bs_estimates_node_ages.txt
